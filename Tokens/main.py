...
def get_script(uri):
    if os.path.isfile(uri):
        ...
        script = json.load(
            open(
                f"{const.ROOT_DIR}resources\\scripts\\{uri}.json", "r", encoding="utf8"
            )
        )
    else:
...
def main():
    ...
    script = script[:]
    gen_sheets_for_script(script, sheet_path, script_name="all_roles_en_copies")
